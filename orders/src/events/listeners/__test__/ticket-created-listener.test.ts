...
import { TicketCreatedEvent } from '@idigitalbeatzgittix/common';
...
it('creates and saves a ticket', async () => {
  const { listener, data, msg } = await setup(); // we dnt need it as async await function in these test but we will in the future

  // call the onMessage function with the data object + message object
  await listener.onMessage(data, msg);

  // write assertion to make sure ticked was created
